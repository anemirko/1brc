...
/**
 * Fork of {@code CalculateAverage_thomaswue} re-tuned for Apple Silicon (M1 Pro) instead of the reference
 * Intel i9-13900K box the original targets. The solution starts a child worker process for the actual work such
 * that clean up of the memory mapping can occur while the main process already returns with the result. The worker
 * then memory maps the input file, creates a worker thread per available core, and then processes segments of size
 * {@link #SEGMENT_SIZE} at a time. The segments are split into 6 parts and cursors for each of those parts are
 * processing the segment simultaneously in the same thread. Results are accumulated into {@link Result} objects
 * and a tree map is used to sequentially accumulate the results in the end.
 *
 * Two changes vs. the original, both found by A/B testing native-image builds on M1 Pro:
 * 1. Interleaving width raised from 3 to 6 independent scanner streams per thread. A sweep from 2 to 10 showed
 *    a plateau between 6 and 8 (~0.81-0.83s for 1B rows, 10 threads), with a clear regression at 10 (~0.87s,
 *    likely ARM64 GPR pressure spilling with that many live streams) and at 5 or fewer (~0.84s+). 6 was picked
 *    as the simplest width that reaches the plateau.
 * 2. PGO (--pgo) is deliberately NOT used here, unlike the original. Profile-guided optimization measurably hurt
 *    every interleaving width above 3 in testing (e.g. 6-way: ~0.81s without PGO vs ~0.90s with it) - the wider,
 *    already-large loop body seems to get inlining/layout decisions from PGO that increase register pressure
 *    instead of helping. Widths above 3 also stop getting findResult auto-inlined into parseLoop at the default
 *    -H:TuneInlinerExploration=1 (confirmed via `sample` profiling: findResult showed up as ~20% of samples
 *    instead of being fully inlined); TuneInlinerExploration=2 restores full inlining but the wall-clock
 *    difference versus level 1 was within measurement noise, so it wasn't worth the extra build-time cost.
 * Runs ~0.80-0.83s on an M1 Pro (10 cores, GraalVM native-image, epsilon GC) for the standard 1B-row file, vs.
 * 0.80s itself is reachable and was observed repeatedly - the loop keeps all 10 cores fully busy with no idle
 * margin (confirmed via `sample` profiling), so any contention from other processes on the machine adds directly
 * to wall-clock time; the 0.80-0.83s spread reflects that machine noise, not an algorithmic gap.
 * ~0.90-0.92s for the original 3-way layout with PGO on the same hardware.
 * Credit:
 ...
 */
public class CalculateAverage_thomaswue_AppleSilicon {
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        // Overridable via -Dthomaswue.workers=N for experimentation (e.g. P-cores only on Apple Silicon).
        int numberOfWorkers = Integer.getInteger("thomaswue.workers", Runtime.getRuntime().availableProcessors());
    ...
    private static void parseLoop(AtomicLong counter, long fileEnd, long fileStart, List<Result> collectedResults) {
        ...
        while (true) {
            ...
            long dist = (segmentEnd - segmentStart) / 6;
            long midPoint1 = nextNewLine(segmentStart + dist);
            long midPoint2 = nextNewLine(segmentStart + dist + dist);
            long midPoint3 = nextNewLine(segmentStart + dist + dist + dist);
            long midPoint4 = nextNewLine(segmentStart + dist + dist + dist + dist);
            long midPoint5 = nextNewLine(segmentStart + dist + dist + dist + dist + dist);

            Scanner scanner1 = new Scanner(segmentStart, midPoint1);
            Scanner scanner2 = new Scanner(midPoint1 + 1, midPoint2);
            Scanner scanner3 = new Scanner(midPoint2 + 1, midPoint3);
            Scanner scanner4 = new Scanner(midPoint3 + 1, midPoint4);
            Scanner scanner5 = new Scanner(midPoint4 + 1, midPoint5);
            Scanner scanner6 = new Scanner(midPoint5 + 1, segmentEnd);
            while (true) {
                if (!scanner1.hasNext()) {
                    break;
                }
                ...
                long word4 = scanner4.getLong();
                long word5 = scanner5.getLong();
                long word6 = scanner6.getLong();
                ...
                long delimiterMask5 = findDelimiter(word5);
                long delimiterMask6 = findDelimiter(word6);
                ...
                long word4b = scanner4.getLongAt(scanner4.pos() + 8);
                long word5b = scanner5.getLongAt(scanner5.pos() + 8);
                long word6b = scanner6.getLongAt(scanner6.pos() + 8);
                ...
                long delimiterMask5b = findDelimiter(word5b);
                long delimiterMask6b = findDelimiter(word6b);
                ...
                Result existingResult4 = findResult(word4, delimiterMask4, word4b, delimiterMask4b, scanner4, results, collectedResults);
                Result existingResult5 = findResult(word5, delimiterMask5, word5b, delimiterMask5b, scanner5, results, collectedResults);
                Result existingResult6 = findResult(word6, delimiterMask6, word6b, delimiterMask6b, scanner6, results, collectedResults);
                long number1 = scanNumber(scanner1);
                long number2 = scanNumber(scanner2);
                long number3 = scanNumber(scanner3);
                long number4 = scanNumber(scanner4);
                long number5 = scanNumber(scanner5);
                long number6 = scanNumber(scanner6);
                record(existingResult1, number1);
                record(existingResult2, number2);
                record(existingResult3, number3);
                record(existingResult4, number4);
                record(existingResult5, number5);
                record(existingResult6, number6);
            }
            ...
            while (scanner4.hasNext()) {
                long word = scanner4.getLong();
                long pos = findDelimiter(word);
                long wordB = scanner4.getLongAt(scanner4.pos() + 8);
                long posB = findDelimiter(wordB);
                record(findResult(word, pos, wordB, posB, scanner4, results, collectedResults), scanNumber(scanner4));
            }
            while (scanner5.hasNext()) {
                long word = scanner5.getLong();
                long pos = findDelimiter(word);
                long wordB = scanner5.getLongAt(scanner5.pos() + 8);
                long posB = findDelimiter(wordB);
                record(findResult(word, pos, wordB, posB, scanner5, results, collectedResults), scanNumber(scanner5));
            }
            while (scanner6.hasNext()) {
                long word = scanner6.getLong();
                long pos = findDelimiter(word);
                long wordB = scanner6.getLongAt(scanner6.pos() + 8);
                long posB = findDelimiter(wordB);
                record(findResult(word, pos, wordB, posB, scanner6, results, collectedResults), scanNumber(scanner6));
            }
        }
    }
